...
export function TrainingPage() {
  ...
  return <Workspace title={user?.candidateType === 'TRADE' ? tr("প্রশিক্ষণ · Your next step") : 'A little learning. More possibility.'} subtitle={tr("Explore practical learning pathways. A referral gives you a next step to discuss with your evaluator; it does not automatically enroll you.")}>
    ...
    {state.data && <div className="space-y-8">
      ...
      <section><h2 className="mb-4 text-xl font-bold">{staff ? 'Referral activity' : tr("My referrals · আমার প্রশিক্ষণ")}</h2><div className="grid gap-4 sm:grid-cols-2">{state.data.referrals.map(r => <article key={r.id} className="surface"><span className="badge">{tr(words(r.status))}</span><h3 className="mt-4 text-lg font-bold">{r.program.title}</h3><p className="mt-2 text-sm text-slate-600">{staff ? `${r.candidate} · ` : ''}{r.program.providerName}</p><p className="mt-3 leading-relaxed text-slate-600">{r.program.description}</p><p className="mt-4 text-sm text-slate-500">{tr("Referred")}{' '}{new Date(r.createdAt).toLocaleDateString()}</p></article>)}</div>{!state.data.referrals.length && <Empty title={tr("Your next chapter starts here")}>{tr("No referrals yet. Your evaluator can recommend a program after reviewing your assessment.")}</Empty>}</section>
      <section><h2 className="mb-4 text-xl font-bold">{tr("Learning catalog")}</h2><div className="grid gap-4 sm:grid-cols-2">{state.data.programs.map(p => <article key={p.id} className="surface"><p className="eyebrow">{p.skill ?? tr("FOUNDATION SKILLS")}</p><h3 className="mt-3 text-xl font-bold">{p.title}</h3><p className="mt-2 text-sm font-medium text-indigo-700">{p.providerName}</p><p className="mt-4 leading-relaxed text-slate-600">{p.description}</p></article>)}</div>{!state.data.programs.length && <Empty title={tr("New programs are on the way")}>{tr("Check back with your evaluator for learning options.")}</Empty>}</section>
    </div>}
  </Workspace>
}
